...
CConfigureDialog::CConfigureDialog() 
	: dialogT(DIALOG_WIDTH, DIALOG_HEIGHT, g_PluginInfo, 10, _T("Settings"))
...
void CConfigureDialog::dde_main_dialog(bool bSaveToControls)
{
	if (bSaveToControls)
	{
		...
		for (int i = 0; i < NUM_MAIN_DW_SETTINGS; ++i)
		{
			...
			{
				m_DialogItems[main_dw_settings[i].control_id].Selected = m_dwParams[main_dw_settings[i].settings_id];
			}
		}
		for (int i = 0; i < NUM_MAIN_STR_SETTINGS; ++i)
		{
			lstrcpy(m_DialogItems[main_str_settings[i].control_id].Data, m_strParams[main_str_settings[i].settings_id].c_str()); //!TODO: OEM ?
		}

	//ST_EDIT_MENU_DISK_FAST_KEY храним как DWORD, показываем как строку
		m_DialogItems[ID_EDIT_MENU_DISK_FAST_KEY].Data[0] = _T('0') + static_cast<int>(m_dwParams[nf::ST_EDIT_MENU_DISK_FAST_KEY]);
		m_DialogItems[ID_EDIT_MENU_DISK_FAST_KEY].Data[1] = 0;
	} else 
	{	//сохраняем настройки из контролов в вектора
		for (int i = 0; i < NUM_MAIN_DW_SETTINGS; ++i)
		{
			if (nf::ST_EDIT_MENU_DISK_FAST_KEY != main_dw_settings[i].settings_id)
			{
				m_dwParams[main_dw_settings[i].settings_id] = m_DialogItems[main_dw_settings[i].control_id].Selected;
			}
		}
		for (int i = 0; i < NUM_MAIN_STR_SETTINGS; ++i)
		{
			m_strParams[main_str_settings[i].settings_id] = static_cast<LPCTSTR>(&m_DialogItems[main_str_settings[i].control_id].Data[0]);
		}
		m_dwParams[nf::ST_EDIT_MENU_DISK_FAST_KEY] = m_DialogItems[ID_EDIT_MENU_DISK_FAST_KEY].Data[0] - _T('0');
	}
}
