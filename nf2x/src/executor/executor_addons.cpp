...
namespace 
{
	...
	{	//получить список <имя переменная среды, путь>, удовлетворяющих шаблону VarPattern
		...
		tstring& mask = (bIncompleteVarPatternPattern && (! bMetachars))
			? //tstring(_T("*")) +  //нет необходимости искать по шаблону "*маска*", шаблон "маска*" удобнее пользователям
			variable_name + tstring(_T("*"))
			: variable_name;
		...
		while (*penv != NULL)
		{
			std::string senv = *penv;
			std::size_t pos = senv.find("=");
			if (pos != tstring::npos)
			{
				assert(pos+1 < senv.size());
				std::string env_name(senv.c_str(), pos);
				std::string pe(senv.c_str(), pos+1, senv.size() - pos);

				if (nf::Parser::IsTokenMatchedToPattern( env_name, mask, false))
				{
					std::list<tstring> list_tokens;
					Utils::SplitStringByRegex(pe, list_tokens, _T(";"));
		...
		, tstring &SubKeyPath)
	{
		...
		if (! lstrcmpi(hkey_path.c_str(), _T("HKEY_CURRENT_USER"))) hkey = HKEY_CURRENT_USER;
		else if (! lstrcmpi(hkey_path.c_str(), _T("HKEY_LOCAL_MACHINE"))) hkey = HKEY_LOCAL_MACHINE;
	...
	{	//путь заносим в виде: имя ключа, путь
		...
		if (! SubkeyPatternName.empty()) SubkeyPatternName += _T("*");
...
{	//получить полный список вариантов именованных директорий 	
	...
	//	if (LocalPath.size()) LocalPath = _T("*") + LocalPath;
	std::list<std::pair<tstring, tstring> > list_var_paths;
										   ...
										   , tstring &DestPath)
{
	...
	if (! LocalPath.empty()) {
		additional_local_path = additional_local_path.empty()
			? LocalPath
			: Utils::CombinePath(additional_local_path, LocalPath, _T("\\"));
		}
	...
	if (! additional_local_path.empty()) DestPath = Utils::CombinePath(DestPath, additional_local_path, _T("\\"));
	return true;
};
											 ...
											 , nf::tshortcut_info& sh)
{
	...
	while (true)
	{
		...
		case nf::QK_DELETE_SHORTCUT_IMPLICIT:
			Shell::SelectShortcutsByPath(_T("")
				, current_path.c_str()
				, list
				, true);
			break;
