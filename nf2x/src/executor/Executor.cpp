...
bool nf::ExecuteCommand(nf::tparsed_command &cmd)
{
	...
	case nf::QK_OPEN_BY_PATH:	//cd:~ 
		...
		if (! nf::Selectors::GetShortcutByPathPattern(plugin
			, plugin.GetPanelInfo(true).CurDir
			, cmd
			, sh)) return false;
		//открыть путь соответствующий псевдониму
	...
	case nf::QK_DELETE_SHORTCUT:	//cD:-
		{
			...
			if (bImplicit)
			{
				...
				//!TODO: проверить 
				if (! nf::Selectors::GetShortcutByPathPattern(plugin
					, Utils::GetOemToInternal(plugin.GetPanelInfo(true).CurDir)
					, cmd
					, sh)) return false;
...
namespace 
{
	tstring get_shortcut_value_ex(CPanelInfoWrap &Plugin, nf::tshortcut_info const& SrcSh)
	{
		...
		if (SrcSh.shortcut == _T(".")) return Plugin.GetPanelInfo(true).CurDir;
...
//удаляем ТОЛЬКО ОДИН лидирующий слеш
		if (LocalPath.size() > 1 && LocalPath[0] == _T('\\')) LocalPath.erase(LocalPath.begin());
...
{	//открыть заданную директорию на активной панели
	...
	::OpenShortcutOnPanel(hPlugin, ap, _T(""), true, false, true, nf::WTS_DIRECTORIES);
	return true;
}
...
{
	tstring path = tstring(_T("explorer \"")) + s + _T("\"");
							   ...
							   , bool bImplicit)
{
	...
	CPanelInfoWrap plugin(hPlugin);
	tstring cur_dir = Utils::GetOemToInternal(plugin.GetPanelInfo(true).CurDir);
											...
											, bool bImplicit)
{
	...
	CPanelInfoWrap plugin(hPlugin);
	tstring cur_dir1 = Utils::GetOemToInternal(plugin.GetPanelInfo(true).CurDir);
	tstring cur_dir2 = Utils::GetOemToInternal(plugin.GetPanelInfo(false).CurDir);
...
nf::Commands::get_implicit_name_and_value(HANDLE hPlugin, bool bGetDataFromActivePanel)
{
	...
	CPanelInfoWrap plugin(hPlugin);
	result.second = Utils::GetOemToInternal(plugin.GetPanelInfo(bGetDataFromActivePanel).CurDir);
