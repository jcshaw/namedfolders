...
namespace nf
{
	...
	inline const char *GetMsg(int MsgId) {return g_PluginInfo.GetMsg(g_PluginInfo.ModuleNumber, MsgId);}
	inline tstring GetRegistryKeyForCommandPatterns()
	{
		return CSettings::GetInstance().get_NamedFolders_reg_key() + tstring(_T("\\CommandPatterns"));
	}};
