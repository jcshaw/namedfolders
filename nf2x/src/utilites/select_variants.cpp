...
namespace
{
		...
		, bool bIsTemporary)
	{
		if (nf::Parser::IsTokenMatchedToPatternOEM(sv.GetName(), PatternOEM))
		{
			...
			info.shortcut = Utils::GetOemToInternal(sv.GetName());
		...
		, tstring Catalog)
	{
		...
		{	//перебираем все вложенные подкаталоги
			...
			while (p != seqc.end())
			{
				...
				tstring subcatalog_name = Utils::GetOemToInternal(cv.GetName());
		...
		, bool bSearchByPath)
	{
		...
		{	//добавляем директорию "поддиректории" в качестве псевдонимов
			...
			tstring current_directory = CPanelInfoWrap(INVALID_HANDLE_VALUE).GetPanelInfo(true).CurDir;
			...
			if (! current_directory.empty())
			{
				...
				tstring pattern = Parser::IsContainsMetachars(ShortcutPatternOEM) 
						? ShortcutPatternOEM
						: ShortcutPatternOEM + _T("*");	//ищем субдиректории как 'b*', а не как '*b*', чтобы исключить лишние срабатывния
				...
				while (p != f.end())
				{
					...
					sh.catalog = _T(".");
					sh.shortcut = tstring(_T("\\")) + Utils::GetOemToInternal((*p).get_filename());
							   ...
							   , bool bSearchInSubcatalogs)
{
	...
	return select_shortcuts_ex(DestList, Utils::GetInternalToOem(shortcut_pattern), bSearchInSubcatalogs, catalog, false);
}
									 ...
									 , bool bSearchInSubcatalogs)
{
	...
	tstring value_pattern_oem = nf::Parser::ConvertToMask(("*")) + Utils::GetInternalToOem(value_pattern) + tstring_oem(_T("*"));
...
namespace
{
	void search_catalogs(sc::CCatalog& c, std::list<tstring> &ListCatalogParts, nf::tcatalogs_list& list)
	{
		...
		while (p != f.end())
		{
			sc::catalogs_sequence_item cv(*p);
			++p;
			tstring catalog_name = Utils::GetOemToInternal(cv.GetName().c_str());
