...
namespace 
{
	...
	{	
		...
		//"1/2/3/" "5/6" -> 1/2/5/6
		//"1/2/3/" "5/6/" -> 1/2/5/6/3
		const tstring ROOT(_T("../"));
		const tstring ROOT_SHORT(_T("/.."));
		...
		Utils::add_trailing_char_if_not_exists(TargetCatalog, SLASH_CATS);
		if (TargetCatalog.size() >= ROOT.size())
		{
			tstring::const_reverse_iterator p1 = TargetCatalog.rend();
			tstring::const_reverse_iterator p2 = ROOT.rend();
			while (p2 != ROOT.rbegin())
			{
				if (*p1 == *p2)
				{
					--p1;
		...
		//сокращаем все ..
		Utils::add_leading_char_if_not_exists(ResultCatalog, SLASH_CATS);
...
{	//найти все ярлыки удовлетворяющие переданному шаблону
	if (!shortcut_pattern.size()) shortcut_pattern = _T("*");
...
{	//добавить новый псевдоним в каталог
	...
	tstring oem_value = Utils::GetInternalToOem(value);
	c.SetShortcut(Utils::GetInternalToOem(sh.shortcut), oem_value, sh.bIsTemporary);
	return true;
}
...
bool nf::Shell::InsertCatalog(tstring catalog, TCHAR const* Parent)
{
	Utils::RemoveTrailingChars(catalog, SLASH_CATS_CHAR);
