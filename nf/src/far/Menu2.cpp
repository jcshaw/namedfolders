...
namespace
{
	inline void safe_copy(wchar_t *dest, tstring const &srcStr, UINT MaxSize)
	{	//скопировать строку в буфер, сжать до заданной ширины
		if (static_cast<UINT>(srcStr.size()) > MaxSize) {
			PathCompactPathEx(dest, srcStr.c_str(), MaxSize, 0);
		} else lstrcpy(dest, srcStr.c_str());
	}
}
...
namespace 
{
	...
	{	
		...
		struct tenv 
		{
			...
			inline static R Decode(V const& SrcValue) { return Utils::CombinePath(SrcValue.first, SrcValue.second, L"%");	} //!TODO
		};
	}
}
...
namespace
{
	...
	wchar_t additional_chars_translation[number_additional_chars] = {'+'
		, ';'
		, 'Ж'
		, 'Б'
		, '-'
...
public:
	...
	bool ShowMenu(tvariant_value &DestValue, int &DestRetCode)
	{
		...
		if ( (! (m_Menu.m_Flags & tmenu::FG_SHOW_SINGLE_VARIANT)) && (1 == m_List.size())) {	//единственный вариант - показывать меню не требуется..
