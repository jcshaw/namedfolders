...
----------------------------------------------------------------------------
Версия 3.0.0.254 release [22.01.2011]
----------------------------------------------------------------------------
[*] Незначительные правки кода.
[*] Доработан хелп.

----------------------------------------------------------------------------
Версия 3.0.0.253 RC2 [16.01.2011]
----------------------------------------------------------------------------
[!] Введены новые метасимволы "\**:N" и "\..*:N", позволяющие
...
[-] Реализована команда перехода в сетевую директорию
"cd:\\network_path". В предыдущих билдах NF3 она не работал в принципе.
[-] Нажатие Esc в процессе выполнения NF-команды приводило к отмене 
команды без показа найденных к моменту отмены результатов. Теперь результаты
показываются.
[-] Не работал переход в сетевые директории с точкой в имени,
например "\\computer\abc.cde"
[-] Команды "cd:%var%" и "cd:%var" в отдельных случаях 
не работали под Windows XP.
[+] Команда "cd:path" теперь позволяет задавать path в кавычках,
например: cd:"c:\temp"
[+] Команды "cd:--f" и "cd:--df" теперь поддерживают возможность
...
----------------------------------------------------------------------------
Версия 3.0.0.249 RC1 [7.01.2011]
----------------------------------------------------------------------------
[*] Команды прямого поиска теперь используют технологию 
обработки относительных путей, т.е. полностью поддерживаются метасимволы
"\**", "..*", "\*", "\..". Команды "cd:z:" и "cd:z:\" обрабатываются как 
"cd:z:\*"
[-] Команда "cd:a/" создавала каталог. Теперь каталог не
создается. Панель с каталогом открывается только если каталог уже существует, 
см #38
[+] Реализованы команды неявного создания шоткатов, ссылающихся
на две панели сразу: "cd:::", "cd::+", "cd:::catalog/", "cd::+catalog/", см
#38
[-] Команда "cd::a/" не создавала каталог "a", если имя каталога
было однобуквенным, см. #38
[-] Нажатие Ctrl+'/' на панели приводило к созданию левых
...
курсор устанавливается на предыдущий каталог, см. #41.
[*] Команды неявного создания плагинов теперь позволяют
...
как команда создания каталога и псевдоним создан не будет.
[*] Команда cd::a/b теперь предлагает создать каталог "a", если
такого каталога нет, а затем создать в нем псевдоним "b". Ранее команда не
срабатывала, если каталог "a" не существовал. См. #20
[-] Исправлено множество ошибок, см. #7, #40, #38.

----------------------------------------------------------------------------
Версия 3.0.0.245 beta [25.12.2010]
----------------------------------------------------------------------------
Ключевые изменения
~~~~~~~~~~~~~~~~~~
[!] Плагин переведен на Far API 2.0 и стал юникодным.
[!] Выпущена 64-битная версия плагина. Теперь плагин доступен в
двух вариантах: для x86 и x64.
[!] Для плагина создан open source проект на google code:
...
снапшот плагина (релизы будут попрежнему выкладываться на PlugRing).
[!] При переходе с NF2 на NF3 русские алиасы превращаются в 
...
затем загнать обратно в реестр.

Исправления
~~~~~~~~~~~
[-] Исправлен запуск 64-битных приложений из 32-битной версии плагина
(#6, http://code.google.com/p/namedfolders/issues/detail?id=6)
[*] Изменена процедура удаления двойных префиксов. Ранее, 
...
как "ef:command".
[*] Изменен синтаксис для поиска в локальных директориях. 
...
Одновременно поддерживаются и старый ("..") и новый ("\..") синтаксис.
[+] Добавлена возможность редактировать шоткаты из меню выбора
вариантов. Редактирование вызывается клавишей F4. В связи с этим, изменена
клавиша переключения на режим просмотра "алиасы и значения" с F4 на F7.
[+] Изменен макрос FastPrefix. Нажатие ':' в командной строке
...
команду NF на английском языке.
[+] Меню вариантов теперь показывает общее количество элементов
в меню
[!] Полностью переделана технология обработки относительных путей.
...
[*] Теперь, если по команде "cd:a\path" плагину не удалось
...
Настройки плагина
~~~~~~~~~~~~~~~~~
[*] Убрана возможность задавать клавиша быстрого запуска для 
плагина в меню дисков. После выхода FAR 2.0 build 1692 она стала 
попросту неактуальной. Issue #15
[+] Добавлен флаг "Показывать каталоги в меню дисков". Если
этот флаг включен, то в меню дисков, помимо строки "Named Folders", появятся
все NF-каталоги. Это экспериментальная реализация Issue #8.
[+] Добавлен флаг "Краткие команды в глубоком поиске". Если
